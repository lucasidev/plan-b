...
Scaffoldeás un feature backend siguiendo el vertical slice del monolito. El patrón completo vive en [`backend/AGENTS.md`](../../../backend/AGENTS.md) (que ya se carga cuando trabajás en `backend/`); este skill es el **procedimiento + el ejemplo canónico a copiar**. No reinventes la estructura de memoria: abrila de un slice real y espejala. El código es la verdad, no esta guía.
