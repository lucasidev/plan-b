...
Scaffoldeás un feature frontend siguiendo el flat slice por caso de uso. El patrón completo vive en [`frontend/AGENTS.md`](../../../frontend/AGENTS.md); este skill es el procedimiento + los ejemplos canónicos a copiar. No reinventes de memoria: copiá la forma de un feature real.
...
- **UI en español rioplatense**, desde la user story (su "listo cuando" y la persona a la que sirve) y los tokens de `docs/product/design-system.md`. La ficha de la pantalla (`docs/product/<slice>/screens/<screen>/README.md`, con su `sketch.html`) es la referencia visual cuando existe; el mapa (`docs/product/map.md`) es orientativo sobre vistas y flujos, no un contrato de UX/UI. El diseño va primero: si falta backend, se mantiene el visual con stub + TODO, no se vacía la vista.
