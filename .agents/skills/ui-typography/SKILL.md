...
## Convenciones de planb

Usar los tokens de [design-system.md](../../../docs/product/design-system.md) y texto en español rioplatense. Por preferencia explícita de Lucas, no usar el guion largo U+2014: resolver con dos puntos, comas, paréntesis o frases separadas. Esta regla prevalece sobre los ejemplos tipográficos generales de abajo.
...
1. **Actual UTF-8 characters (preferred):** Paste the real character directly into the source file.
   ```jsx
   <p>Don’t do this</p>  {/* Carácter UTF-8 U+2019. */}
   ```
...
3. **HTML entity in HTML or JSX text:** `<p>Don&rsquo;t do this</p>` renders the apostrophe. Entities inside JavaScript string values are literal text and are not decoded.

For bulk edits, preserve UTF-8 and use the project tooling available on the current platform.
