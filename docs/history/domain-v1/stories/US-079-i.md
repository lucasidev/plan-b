...
**Status**: Done
**Sprint**: S4
<!-- TODO(doc): varios AC quedaron sin marcar porque el handler implementado difiere del spec original (documentado en los comentarios XML de ChangePasswordCommandHandler.cs): revoca TODOS los refresh tokens (no "todos excepto el actual"), sin rate limiter, sin integration event (solo domain event), sin audit log todavía. Ver el propio código para el razonamiento. -->
**Epic**: [EPIC-02: Identidad y autenticación](../epics/EPIC-02.md)
**Priority**: Medium
**Effort**: S (slice integrated chico, sigue patrón de US-029-i / US-033-i)
**ADR refs**: [ADR-0023](../../../decisions/0023-auth-flow-jwt-cookie-layout-guards.md), [ADR-0034](../../../decisions/0034-redis-as-cache-and-ephemeral-state.md), [ADR-0040](../../../decisions/0040-notifications-as-a-new-bounded-context.md), [ADR-0041](../../../decisions/0041-ux-redesign-after-claude-design.md)
...
- [x] `PATCH /api/me/password` con `{ currentPassword, newPassword }`.
- [x] Requiere sesión válida (`planb_session` cookie con JWT activo).
- [x] Valida `currentPassword` contra el hash BCrypt persistido del user. Si no matchea: 401 `identity.password.current_invalid` (sin distinguir entre "no matchea" vs "user no existe" para evitar enumeration).
- [x] Valida `newPassword` server-side:
  - Mínimo 8 chars.
...
- [x] Hash BCrypt nuevo con work factor 12.
...
- [x] Longitud máxima de `newPassword`: 200 chars (sane upper bound, no security issue real porque BCrypt trunca a 72 bytes pero queremos validación explícita).

### Frontend

- [x] **Modal "Cambiar contraseña"** (port literal de `canvas-mocks/v2-modals.jsx::V2ModalCambiarContrasena`, captura `cuenta-v2-modal-pass.png`):
...
- [x] **Trigger**: desde `/ajustes` (US-072), sección "Seguridad", row "Contraseña" con valor "Cambiar contraseña →" abre el modal.
...
- [x] **Sin show/hide password toggle** en MVP (deuda menor; se agrega si Lucas lo pide).
...
- **Audit log integrado**: depende de US-086 (aterriza en PR siguiente del módulo backoffice) que crea `identity.user_audit_log`. Mientras tanto, log estructurado a stdout cubre el caso.
...
- [x] Comando `ChangePasswordCommand` + `ChangePasswordCommandHandler` en `Planb.Identity.Application/Features/ChangePassword/`.
- [x] Validator con FluentValidation (length + distinct + OAuth check).
- [x] BCrypt verify + rehash (work factor 12 alineado a sign-up).
...
- [x] Endpoint Carter `PATCH /api/me/password`.
- [ ] Audit log entry (cuando US-086 esté disponible; mientras tanto structured log).
- [x] Integration tests.

### Frontend

- [x] `features/change-password/{actions.ts,schema.ts,components/change-password-modal.tsx,types.ts}`.
- [x] Zod schema compartido client + server (length, distinct, confirm matches).
- [x] Update `/ajustes` (US-072) para que el row "Contraseña" abra este modal.
...
- **Revocar refresh excepto el actual**: el handler tiene el `sessionTokenHash` del JWT actual (viene en el claim del cookie). Se pasa al `IRefreshTokenStore` como exception list. Si el método no existe todavía, extenderlo en este requerimiento.
...
- **Soft dep (no bloqueante)**: [US-077-b](US-077-f.md) backend de notifications (para el email de aviso) + US-086 (aterriza en PR siguiente del módulo backoffice) audit log per-user (para la entry de audit). Esta US emite el event y loguea; cuando US-077-b / US-086 aterricen, los subscribers se enchufan sin cambiar este flow.
- **Relacionada con**: [US-029-i](US-029-i.md) (mismo patrón de slice integrated), [US-033-i](US-033-i.md) (reset por email, flow hermano), [US-058](US-058.md) (disable user revoca refresh tokens también; mecanismo compartido).

## Refs

- DoD: [Definition of Done](../../../plan/definition-of-done.md)
- Mockup: ![Cambiar contraseña](../../design-v1/reference/screenshots/cuenta-v2-modal-pass.png). Fuente JSX en `canvas-mocks/v2-modals.jsx::V2ModalCambiarContrasena`.
- ADRs: [ADR-0023](../../../decisions/0023-auth-flow-jwt-cookie-layout-guards.md), [ADR-0034](../../../decisions/0034-redis-as-cache-and-ephemeral-state.md), [ADR-0040](../../../decisions/0040-notifications-as-a-new-bounded-context.md), [ADR-0041](../../../decisions/0041-ux-redesign-after-claude-design.md).
- US relacionadas: [US-072](US-072.md), [US-033-i](US-033-i.md), [US-029-i](US-029-i.md), [US-058](US-058.md), [US-077-f](US-077-f.md).
