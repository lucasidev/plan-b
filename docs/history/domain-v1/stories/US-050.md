...
**ADR refs**: [ADR-0009](../../../decisions/0009-review-anonymity-is-a-presentation-rule.md), [ADR-0041](../../../decisions/0041-ux-redesign-after-claude-design.md)
...
> (`ListReportQueueEndpoint.cs`).

- [x] `GET /api/moderation/reports/queue` retorna **reports** (no reviews) con `status = 'open'`, ordenados por `tone DESC` (urgent → normal → low) y luego `created_at ASC` (antigüedad).
- [ ] Cada item del listado:
  ...
  > TODO(doc): el enum real `ReviewReportReason` (`Planb.Moderation.Domain.Reports`) solo tiene 5 valores (`OffTopic`, `DatosPersonales`, `LenguajeInapropiado`, `Difamacion`, `Spam`), no los 11 de este AC. El campo `since` tampoco viene precomputado del backend (`ReportQueueItem` solo expone `CreatedAt`). El resto de los campos del item sí están.
- [x] **Filtros opcionales** en query:
  ...
  > TODO(doc): `?assignedTo=` no está presente ni siquiera como param reservado en `ListReportQueueEndpoint.cs`; `status`, `tone` y `olderThan` sí están implementados.
- [x] **Paginación**: default 20, máximo 100.
- [ ] **Identidad del autor de la reseña visible al moderator** (no anonimizado al staff). ADR-0009.
  > TODO(doc): en el listado (`ReportQueueItem`) no viaja `authorUserId`. Sí está en el detalle (US-051, `ReportDetailResponse.AuthorUserId`).
- [x] **Identidad del reporter visible al moderator** (para detectar abuse patterns: mismo reporter denunciando masivamente).
- [x] Header del read model (separado del listing) con counts globales para los filter chips:
...
- [x] Ruta `/admin/moderacion/reportes` en route group `(staff)`.
- [x] **Page header** (port de `AdmReportesCola`):
...
- [x] **Filter chips** (port de `AdmFilterChip`):
...
- [x] **Vacío**: si `items.length === 0` y `openCount === 0`, mostrar "Sin reportes abiertos. Andá a tomar mate." (empty state celebratorio).
...
- [x] Read model Dapper cross-schema (Moderation reports JOIN Reviews para snippet/target + Identity users para reporter). OK por ADR-0017 (read no requiere FK).
- [ ] `IReportToneClassifier` domain service que mapea `reason` → `tone` (config-driven, no hardcoded en el handler).
  > TODO(doc): implementado distinto. El mapping vive como `CASE WHEN` inline en `DapperReportQueueReader.cs` (read model / presentation), no como domain service ni config-driven.
- [x] Endpoint Carter `GET /api/moderation/reports/queue` con paginación + filtros.
- [x] Devolver `counts` en el response top-level.
- [x] Authorization policy `moderator|admin`.
- [x] Tests integration: sort por tone + antigüedad, filters, paginación, counts, identidad reporter+autor visible al staff.

### Frontend

- [x] `app/(staff)/admin/moderacion/reportes/page.tsx` (server component con prefetch).
- [x] `features/admin-reports-queue/{api.ts,components/{reports-table,tone-dot,filter-chips,page-header}.tsx,types.ts}`.
  > TODO(doc): la feature real vive en `features/moderate-reports/` (no `admin-reports-queue`), con los mismos componentes (`reports-table.tsx`, `tone-dot.tsx`, `filter-chips.tsx`).
- [ ] Reusar `<AdmShell>` + `<AdmTable>` + `<AdmFilters>` de `components/layout/admin-*` (cuando aterricen con US-081).
  > TODO(doc): `AdmTable`/`AdmFilters` no existen en el código (grep sin resultados); la feature implementa su propia tabla y filtros standalone.
...
- **Relacionada con**: [US-053](US-053.md) (audit log de decisiones), [US-058](US-058.md) (disable user desde el detalle del reporter).
...
- Mockup admin canvas (sección ③):
  - ![Reportes · cola](../../design-v1/reference/screenshots/admin-mod-adm-rep-list.png)
  - Fuente JSX en `canvas-mocks/admin-screens-3.jsx::AdmReportesCola`.
- ADRs: [ADR-0009](../../../decisions/0009-review-anonymity-is-a-presentation-rule.md), [ADR-0041](../../../decisions/0041-ux-redesign-after-claude-design.md).
- US relacionadas: [US-019](US-019.md), [US-051](US-051.md), [US-053](US-053.md), [US-058](US-058.md), [US-081](US-081.md).
