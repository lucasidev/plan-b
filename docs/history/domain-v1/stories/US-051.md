...
**ADR refs**: [ADR-0011](../../../decisions/0011-cascade-on-uphold-with-no-reversal-on-restore.md), [ADR-0041](../../../decisions/0041-ux-redesign-after-claude-design.md)
...
> corresponde (acá, `moderation`). Rutas corregidas contra el código real
> (`GetReportDetailEndpoint.cs`, `ResolveReportEndpoints.cs`).

- [ ] `GET /api/moderation/reports/{id}` retorna detalle completo del report:
  ...
  > TODO(doc): `ReportDetailResponse` (código real) no trae `body` completo de la reseña, ni `verified`, ni `targetTerm`/`targetCommission`, ni `authorAnonHandle`, ni el 3er rating (comisión); solo expone `SubjectText`/`TeacherText` + 2 ratings (`DifficultyRating`, `OverallRating`). El resto de las secciones (reporter, author context agregado, otros reports open) sí están.
- [x] `POST /api/moderation/reports/{id}/uphold` con `{ resolutionNote }`:
  ...
  > TODO(doc): el evento real que publica `UpholdReportCommandHandler` es `ReviewRemovalRequestedIntegrationEvent` (Moderation le pide a Reviews que remueva; Reviews es quien posee la transición y su propio audit log), no `ReviewRemoved` directo.
- [x] `POST /api/moderation/reports/{id}/dismiss` con `{ resolutionNote }`:
  ...
  > TODO(doc): `DismissReportCommandHandler` publica `ReviewReportsResolvedIntegrationEvent` cuando no quedan reports open (la decisión de volver a `published` la toma el módulo Reviews, que la owns); no verificado contra el código de Reviews si emite exactamente `ReviewRestored`.
- [ ] Distinción `filter` vs `threshold` (`review_audit_log.transition_reason`): controla approve. Vuelve a published solo si la transición fue `threshold`. Si fue `filter`, queda en `under_review` hasta moderation explícita.
  > TODO(doc): no verificado contra el código. Esta lógica, si existe, vive del lado del handler de Reviews que consume `ReviewReportsResolvedIntegrationEvent`, fuera del módulo Moderation revisado acá.
- [x] Longitud máxima `resolutionNote`: 1000 chars. Mínimo 0 (puede ser vacío).
- [x] Requiere `role IN ('moderator', 'admin')`.
- [x] Idempotente: uphold/dismiss sobre report ya resuelto retorna 409 `moderation.report.already_resolved` con el outcome anterior.

### Frontend

- [x] Ruta `/admin/moderacion/reportes/{reportId}` en route group `(staff)`.
- [x] **Page header** (port de `AdmReporteDetalle`):
...
- [x] **Layout dos columnas** (`1.6fr 1fr`, gap 14px):
...
- [x] **Card "Contexto del autor"** (full width, debajo del grid):
...
- [x] **Banner especial: review borrada por el autor**: si `review.deletedAt != null`, agregar banner amarillo arriba del card de la reseña: "Reseña borrada por el autor. Igual podés desestimar el report con nota." El uphold queda disabled, solo dismiss disponible.
- [x] **Confirmación antes de aplicar**: modal con resumen ("Vas a {aprobar|ocultar} esta reseña. {Esto cerrará {N} reports adicionales por cascade.} La decisión queda en el audit log.") + botones `Cancelar` / `Aplicar`. Solo cuando hay confirmación explícita el POST se dispara.
...
- **Ban manual con motivo y plazo desde el detalle del report**: US-085. La US existente [US-058](US-058.md) cubre el endpoint backend del disable, pero el "Ocultar + banear" del panel de decisión integra el workflow de moderación con el ban en un solo paso, que es lo que aterriza en US-085.
...
- [x] Read model Dapper cross-schema para el detalle (Moderation reports + Reviews + Identity users + Academic teachers/subjects/terms).
- [x] Comando `UpholdReportCommand` + handler con cascade.
- [x] Comando `DismissReportCommand` + handler con check filter vs threshold.
- [x] Endpoints Carter (`GET /api/moderation/reports/{id}`, `POST /api/moderation/reports/{id}/uphold`, `POST /api/moderation/reports/{id}/dismiss`).
- [ ] Audit log entries per-BC (ADR-0042): `ReviewAuditLog` (acción sobre la review), `ModerationActionLog` (acción del moderator sobre el report).
  > TODO(doc): no verificado contra el código. No se encontró un tipo `ModerationActionLog` en el módulo.
- [ ] Cross-BC events vía Wolverine outbox (ADR-0030): `ReviewRemoved`, `ReviewRestored`.
  > TODO(doc): los eventos reales son `ReviewRemovalRequestedIntegrationEvent` (uphold) y `ReviewReportsResolvedIntegrationEvent` (dismiss cuando no quedan reports open); nombres distintos a los del AC. Ver detalle en los AC de arriba.
- [x] Authorization policy `moderator|admin`.
- [x] Tests integration: cascade, dismiss único filter vs threshold, idempotencia, author context agregado correcto.

### Frontend

- [x] `app/(staff)/admin/moderacion/reportes/[reportId]/page.tsx` (server component con prefetch).
- [x] `features/admin-report-detail/{api.ts,components/{review-card,decision-panel,author-context-card,apply-decision-modal,page-header}.tsx,types.ts}`.
  > TODO(doc): la feature real vive en `features/moderate-reports/` (misma carpeta que US-050), con los mismos componentes (`review-card.tsx`, `decision-panel.tsx`, `author-context-card.tsx`, `apply-decision-modal.tsx`).
- [x] Mapping radio → endpoint (Aprobar→dismiss, Ocultar reseña→uphold, otros→disabled).
- [ ] Reusar `<AdmShell>` + `<AdmCard>` de `components/layout/admin-*` (US-081).
  > TODO(doc): `AdmCard` no existe en el código (grep sin resultados); la feature implementa sus propios cards standalone.
- [x] Tests vitest + spec E2E.
...
- **Audit log per-BC**: ADR-0042. Cada BC owns su projection. La vista cross-BC del audit (US-086, US-005) se arma con Dapper UNION ALL.
...
- **Relacionada con**: [US-053](US-053.md) (audit log de la decisión), [US-058](US-058.md) (disable user es el endpoint backend que US-085 va a usar para el "Ocultar + banear"), [US-086](US-086.md) (audit log per-user lee de ModerationActionLog).
...
- Mockup admin canvas (sección ③):
  - ![Reporte · detalle con 5 opciones](../../design-v1/reference/screenshots/admin-mod-adm-rep-det.png)
  - Fuente JSX en `canvas-mocks/admin-screens-3.jsx::AdmReporteDetalle`.
- ADRs: [ADR-0011](../../../decisions/0011-cascade-on-uphold-with-no-reversal-on-restore.md), [ADR-0041](../../../decisions/0041-ux-redesign-after-claude-design.md), [ADR-0042](../../../decisions/0042-audit-log-per-bc-not-central.md).
- US relacionadas: [US-019](US-019.md), [US-050](US-050.md), [US-052](US-052.md), [US-053](US-053.md), [US-058](US-058.md), [US-085](US-085.md), [US-086](US-086.md).
