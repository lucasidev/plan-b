...
- [x] Ruta `/ajustes` (route group `(member)`).
- [x] Acceso desde **ítem "Ajustes" en sección "Otros"** del sidebar v2.
...
- [x] Cada cambio se guarda automáticamente al toggle/select (no botón "Guardar"; UX de settings modernos).
- [x] **Sección "Seguridad"** con row "Contraseña" + valor "Cambiar contraseña →" como link/trigger del modal de **[US-079-i](US-079-i.md)** (cambio de contraseña con sesión activa, slice integrated propio: endpoint backend + modal frontend). Esta US **solo monta el row** en Ajustes; el modal + endpoint + validaciones + revocación de refresh tokens + notification al user viven en US-079-i.
...
- [x] `GET /api/users/me/settings` (devuelve current settings).
- [x] `PATCH /api/users/me/settings` con cualquier subset.
- [x] Storage en `user_settings` table (FK a User, JSONB con shape validado por Zod / FluentValidation server side).
- [x] Tests integration.

### Frontend

- [x] `app/(member)/ajustes/page.tsx`.
- [x] `features/settings/{api.ts,actions.ts,schema.ts,components/{notifications-section,privacy-section,language-select,theme-select}.tsx}`.
- [x] Theme provider que lee del setting (next-themes o similar).
...
- [x] Sidebar v2: agregar "Ajustes" en sección "Otros".
...
- Mockups:
  - ![Ajustes](../../design-v1/reference/screenshots/cuenta-v2-ajustes.png). Fuente JSX en `canvas-mocks/v2-screens-2.jsx::V2Ajustes` líneas 605-708.
