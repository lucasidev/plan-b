...
**Status**: Subdividida. **No se trackea acá**: el estado real vive en [US-044-a](US-044-a.md) + [US-044-b](US-044-b.md) + [US-044-c](US-044-c.md)
**Sprint**: S2
...
>
> Este archivo describe el alcance integral; los requerimientos contienen los AC concretos por bloque, sub-tasks y tests filtrados.
...
- [ ] **Empty state global del Inicio** (`v2-empty.jsx::V2InicioVacio`, captura `home-v2-inicio-empty`): cuando el alumno tiene StudentProfile pero todavía no tiene período cargado (ningún `EnrollmentRecord` ni `Simulation` activa), reemplaza el grid 2-col por una card central con eyebrow "Inicio" + heading "Tu plan-b está vacío todavía." + checklist de próximos pasos (1. cargá tu historial, 2. armá un borrador, 3. activalo cuando arranque el cuatri) + CTA primary "Crear borrador 2026".
...
| User con StudentProfile pero sin período cargado | Renderea el empty state global `V2InicioVacio` (no el grid 2-col). |
...
- Mockups (Inicio + estados):
  - ![Inicio v2](../../design-v1/reference/screenshots/home-v2-inicio.png).
  - ![Inicio · vacío (alumno nuevo)](../../design-v1/reference/screenshots/home-v2-inicio-empty.png).
  - Fuente JSX en `canvas-mocks/v2-screens.jsx::V2Inicio` + `canvas-mocks/v2-empty.jsx::V2InicioVacio` + datos en `v2-shell.jsx` (V2_USER, V2_PERIOD, V2_ACTIVE, V2_TO_REVIEW).
  - El estado offline del Inicio (`home-v2-inicio-offline`) lo cubre [US-039-f](US-039-f.md), no esta US.
