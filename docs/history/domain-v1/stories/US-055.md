...
- [x] `DELETE /api/me/reviews/{reviewId}` con ownership check (autor === sesión actual).
- [x] Si la review está en `under_review` o `published`: pasa a `deleted` (soft delete) con `deletedAt` + `deletedReason='self'`. Mantenemos el row para audit.
- [x] Si la review estaba `published` y aparecía en feed/rankings: re-projecta para que desaparezca de los reads.
- [x] Emite `ReviewDeleted` via Wolverine outbox para invalidar caches y feeds.
- [x] Idempotencia: segunda llamada con mismo `reviewId` devuelve 200 con el body de la review ya borrada.
- [x] Audit log entry con `action='deleted'`, `actor=author`, `reason='self'`.

### Frontend

- [x] CTA "Borrar" disponible en:
  - El menú contextual de cada reseña en `/reseñas?tab=mias`.
  - El header del editor de reseña (US-049) cuando está en modo edit.
- [x] **Modal destructivo** (port literal de `v2-modals.jsx::V2ModalBorrarResena`):
...
- [x] Después de borrar exitoso: toast "Reseña borrada" + redirect a `/reseñas?tab=mias` + invalidar query `['reviews', 'mine']`.
...
|---|---|
| User borra una reseña que aparecía en rankings (US-057) | Re-projecta rankings para excluirla. Cambio se ve en próximo render del ranking. |
...
- [x] Comando `DeleteOwnReviewCommand` + handler en módulo Reviews.
- [x] Endpoint Carter `DELETE /api/me/reviews/{reviewId}`.
- [x] Soft delete (campo `DeletedAt` + `DeletedReason` en `Review` aggregate).
- [x] Re-projector de feeds + rankings cuando llega `ReviewDeleted`.
- [x] Tests integration.

### Frontend

- [x] `features/delete-review/{actions.ts,api.ts,schema.ts,components/delete-review-modal.tsx,types.ts}`.
- [x] Reusar el patrón `typed-confirm` del `delete-account-button` (ya existe).
- [x] Trigger del modal desde `/reseñas?tab=mias` row + desde editor en mode edit.
- [x] Tests vitest unit + component.
- [x] Spec E2E.
...
- Mockup: ![Borrar reseña](../../design-v1/reference/screenshots/resenas-v2-modal-borrar.png). Fuente JSX en `canvas-mocks/v2-modals.jsx::V2ModalBorrarResena`.
- ADRs: [ADR-0041](../../../decisions/0041-ux-redesign-after-claude-design.md), [ADR-0009](../../../decisions/0009-review-anonymity-is-a-presentation-rule.md).
