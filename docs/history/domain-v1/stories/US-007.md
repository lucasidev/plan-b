# US-007: Importador de plan con preview/diff (CSV → catálogo)

**Status**: Superada por BO1-2 y Catálogo (backoffice) (la carga la hace el equipo; si un importador ayuda, se decide en esa ficha). Era: Backlog
...
- [ ] **Endpoint**: `POST /api/academic/plans/{planId}/import` con body `{ csv: string }` que devuelve `{ preview: { row, status, current, incoming, errors[] }[] }` cuando se llama con `?dry=true` (preview) o aplica cuando `?dry=false`.
...
- [ ] Endpoint Carter `POST /api/academic/plans/{id}/import?dry=bool`.
...
- Mockup: ![Importador CSV](../../design-v1/reference/screenshots/admin-datos-adm-imp.png). Fuente JSX en `canvas-mocks/admin-screens-1.jsx::AdmImportador`.
