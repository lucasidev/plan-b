...
US-T04-b introdujo `Planb.ArchitectureTests` con 7 reglas para el modulo Identity. Los otros 4 modulos (Academic, Reviews, Moderation, Enrollments) ya tienen codigo real desde los requerimientos de S2-S5 y no tienen coverage de reglas de arquitectura: un endpoint que inyecte `DbContext` directamente, o un handler que referencie `Microsoft.AspNetCore.*`, pasaria CI sin ninguna alarma.
