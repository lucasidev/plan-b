...
**ADR refs**: [ADR-0023](../../../decisions/0023-auth-flow-jwt-cookie-layout-guards.md), [ADR-0041](../../../decisions/0041-ux-redesign-after-claude-design.md)

> Esta US es de **rediseño puro** (sin cambios de comportamiento ni de backend) + **generalización de copy UNSTA→multi-universidad** (sumado 2026-07-15, ver AC "Copy multi-universidad"). Cubre las 4 vistas auth (`sign-up`, `sign-in`, `forgot-password`, `forgot-password/check-inbox`) y las 4 vistas onboarding (`welcome`, `career`, `history`, `done`) que hoy viven en código (US-010-f / US-028-f / US-033-i / US-037-f, todas Done en S1) pero usan layouts previos al canvas v2 cerrado en la sesión de claude-design del 2026-05-02.
...
### Copy multi-universidad (generalización UNSTA-first, sumado 2026-07-15)

El data model ya es multi-universidad (US-060 aterrizó el CRUD de University, 4 unis reales), pero varios strings user-facing quedaron UNSTA-first del canvas. Al construir los shells nuevos se generalizan:

- [ ] **Hero de auth**: la copy de `components/layout/auth-hero-data.ts` (que esta US borra junto con `auth-hero.tsx`) no se re-hardcodea con "alumnos de UNSTA"; el `AuthShell` nuevo usa copy multi-universidad. Los stats ("340 alumnos verificados / **3 universidades** / 1.2k reseñas") se corrigen al conteo real o se dejan como placeholder honesto hasta `GET /api/stats/public`: hoy dicen "3 universidades" y el catálogo tiene 4.
- [ ] **Badge institucional**: `components/ui/verified-badge.tsx` hardcodea `teacher: 'docente UNSTA'`. Generalizar a un label agnóstico ("docente verificado") o parametrizado por la uni del docente.
- [ ] **Chrome del alumno**: `app/(member)/layout.tsx` hardcodea "UNSTA · Carrera" (deuda US-012). Reemplazar por la universidad real del `StudentProfile`.
- [ ] **Dirección: generalizar, no conservar.** El mock es UNSTA-first, pero el producto es multi-universidad: se generaliza **toda** la copy de producto. Hasta el disclaimer legal se vuelve genérico ("proyecto independiente, no afiliado oficialmente a ninguna universidad") en vez de nombrar UNSTA. La marca UNSTA no es el default de ningún string; que el canvas la tenga hardcodeada no la vuelve spec.
...
- [ ] `features/sign-up/components/carnet-preview.tsx` (port de `auth.jsx::CarnetPreview` líneas 127-237): muestra dos carnets ("privado" + "público anónimo") side-by-side stack vertical. **Puede ser estático en MVP** (no conectarse al estado del form en este requerimiento). Si después se quiere conectarlo (live preview del nombre que se está tipeando), entra en US-XXX-f separada.
...
- **Relacionada con**: [US-054-f](US-054-f.md) (la landing comparte el lenguaje visual; el shell de auth debe sentirse continuación de la landing), [ADR-0041](../../../decisions/0041-ux-redesign-after-claude-design.md).

## Refs

- DoD: [Definition of Done](../../../plan/definition-of-done.md)
- Mockups (canvas v2):
  - ![Sign up](../../design-v1/reference/screenshots/auth-signup.png) (panel izq: `CarnetPreview`).
  - ![Sign up · email duplicado](../../design-v1/reference/screenshots/auth-signup-err.png) (banner inline `AuthErrorBanner`).
  - ![Sign in](../../design-v1/reference/screenshots/auth-login.png) (panel izq: `LastActivityPanel`).
  - ![Sign in · credenciales inválidas](../../design-v1/reference/screenshots/auth-login-err.png) (banner inline `AuthErrorBanner`).
  - ![Forgot password](../../design-v1/reference/screenshots/auth-forgot.png) (panel izq: `FlowSteps` paso 1).
  - ![Forgot link sent](../../design-v1/reference/screenshots/auth-forgot-ok.png) (panel izq: `FlowSteps` paso 2).
  - ![Onboarding · welcome](../../design-v1/reference/screenshots/onb-welcome.png).
  - ![Onboarding · career](../../design-v1/reference/screenshots/onb-career.png).
  - ![Onboarding · history](../../design-v1/reference/screenshots/onb-history.png).
  - ![Onboarding · done](../../design-v1/reference/screenshots/onb-done.png).
...
- ADRs: [ADR-0023](../../../decisions/0023-auth-flow-jwt-cookie-layout-guards.md), [ADR-0041](../../../decisions/0041-ux-redesign-after-claude-design.md).
