...
**Status**: Superada por BO3-2 y BO6-1 (quién hizo cada cosa; el registro se revisa). Era: Backlog
**Sprint**:
...
[ADR-0042](../../../decisions/0042-audit-log-per-bc-not-central.md) decidió **per-BC** (cada módulo mantiene su projection local). Esta US implementa el read model Dapper cross-schema que UNION-ea las projections per-BC filtradas por `actor_id = userId OR target_user_id = userId`, ordena por `at DESC` y pagina.

## Acceptance Criteria

> El backoffice no tiene namespace propio: cada feature admin vive en la API de su módulo. Ver [ADR-0050](../../../decisions/0050-backoffice-is-a-cross-cutting-slice-not-a-module.md). `User` es agregado del módulo **identity**: la ruta va ahí (el read model sigue siendo cross-schema vía Dapper por dentro).

### Backend

- [ ] **Endpoint** `GET /api/identity/users/{userId}/audit-log` con paginación (default 30 items, máximo 100). Query params:
...
- [ ] **Shape uniforme** del read model (proyección final):
  ...
  ```
- [ ] **Identidad visible al staff** ([ADR-0009](../../../decisions/0009-review-anonymity-is-a-presentation-rule.md)): el moderator ve nombre real del actor + del target. Anonimato es solo en presentation pública.
...
- **Feed global del dashboard admin**: cubierto por [US-005](US-005.md). El feed mezcla audits de muchos users; esta US es siempre filtrada por un user específico.
...
- **Edición / borrado de entries**: append-only ([ADR-0031](../../../decisions/0031-review-audit-log-as-a-projection.md)). Esta US es read-only.
...
- [ ] **Projectors Wolverine** para cada BC nuevo:
  ...
  - `Academic.CatalogAuditLogProjector` (uni created/archived, plan imported via US-007, subject merged via US-006, plan migration applied via US-084, etc.).
- [ ] **Read model Dapper cross-schema**: `IUserAuditLogReader.GetByUser(userId, cursor, limit, filters)` retorna shape uniforme.
- [ ] **Endpoint** Carter `GET /api/identity/users/{userId}/audit-log`.
...
- [ ] `app/(staff)/admin/moderacion/usuarios/[userId]/audit-log/page.tsx` (probablemente como sub-tab del detalle de user de US-058).
...
- **El patrón de projections per-BC viene de [ADR-0031](../../../decisions/0031-review-audit-log-as-a-projection.md) extendido por [ADR-0042](../../../decisions/0042-audit-log-per-bc-not-central.md)**. Cada BC nuevo replica el patrón. No hay módulo central de Audit.
...
- **Depende de**: [ADR-0042](../../../decisions/0042-audit-log-per-bc-not-central.md), [ADR-0031](../../../decisions/0031-review-audit-log-as-a-projection.md), [US-058](US-058.md) (detalle de usuario donde se monta el tab), [US-081](US-081.md) (admin shell + tabs).
- **Bloquea a**: [US-058](US-058.md) frontend completo (el tab "Audit log" es uno de los tabs del detalle).
- **Relacionada con**: [US-053](US-053.md) (audit log per-review, hermano del patrón), [US-005](US-005.md) (feed global del dashboard, comparte projections), [US-085](US-085.md) (strikes / bans que pueblan `user_audit_log` y `moderation.action_log`).

## Refs

- DoD: [Definition of Done](../../../plan/definition-of-done.md)
- Mockup admin canvas (sección ③):
  - ![Usuario · detalle con tab Audit log](../../design-v1/reference/screenshots/admin-mod-adm-usr-det.png)
  - Fuente JSX en `canvas-mocks/admin-screens-3.jsx::AdmUsuarioDetalle` (tab "Audit log").
- ADRs: [ADR-0042](../../../decisions/0042-audit-log-per-bc-not-central.md), [ADR-0031](../../../decisions/0031-review-audit-log-as-a-projection.md), [ADR-0017](../../../decisions/0017-persistence-ignorance.md), [ADR-0009](../../../decisions/0009-review-anonymity-is-a-presentation-rule.md).
- US relacionadas: [US-053](US-053.md), [US-005](US-005.md), [US-058](US-058.md), [US-085](US-085.md).
