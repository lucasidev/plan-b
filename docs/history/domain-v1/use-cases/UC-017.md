# UC-017: Publicar reseña de una cursada

**Actor**: Alumno
**Epic**: [EPIC-05: Sistema de reseñas](../epics/EPIC-05.md)
**User Story**: [US-017](../../../domain/user-stories/US-017.md)
**ADR refs**: [ADR-0005](../../../decisions/0005-reseña-anclada-al-enrollment.md), [ADR-0007](../../../decisions/0007-pgvector-implementado-ui-gated-off.md), [ADR-0009](../../../decisions/0009-anonimato-como-regla-de-presentacion.md)

## Pre-condiciones

- Tiene un `EnrollmentRecord` con `status != 'cursando'`.
- No tiene ya una `Review` para ese enrollment.

## Flujo principal

El alumno aporta información para futuros cursantes. La publicación es el motor del crowdsourcing y el loop de reciprocidad.

1. Alumno abre la cursada elegida (un EnrollmentRecord finalizado).
2. Sistema muestra formulario: `difficulty_rating` (1..5), `subject_text`, `teacher_text`, `final_grade` (si no estaba cargada), selector de `docente_reseñado_id` (constrained a los Teachers del `CommissionTeacher` de la comisión del enrollment).
3. Alumno completa al menos uno de los dos textos, elige difficulty y docente_reseñado.
4. Sistema valida invariantes (rating en rango, grade en rango, textos combinados no vacíos, docente pertenece a la comisión).
5. Sistema ejecuta el filtro automático sobre `subject_text` y `teacher_text`.
6. Si el filtro no marca, `Review.status='published'`, `created_at=now()`.
7. (Diseño diferido, [ADR-0007](../../../decisions/0007-pgvector-implementado-ui-gated-off.md)): cuando exista el pipeline, el sistema va a encolar un job async para computar `ReviewEmbedding`. Hoy este paso no ocurre.
8. Sistema registra `ReviewAuditLog` con `action='published'`.

## Post-condiciones

- Reseña visible en página de la materia y del docente, con anonimato del autor.

## Flujos alternativos

- **A1 (paso 5)**: si el filtro marca (insultos obvios, longitudes anómalas, links sospechosos), `Review.status='under_review'` y no se expone públicamente hasta que un moderador la revise.
- **A2 (paso 4)**: si `docente_reseñado_id` no pertenece a la comisión del enrollment, rechazo (400).
- **A3 (paso 3)**: si ya existe una Review para ese enrollment, redirigir a UC-018.

## Refs

- User Story: [US-017](../../../domain/user-stories/US-017.md)
- Epic: [EPIC-05](../epics/EPIC-05.md)
- ADRs: [ADR-0005](../../../decisions/0005-reseña-anclada-al-enrollment.md), [ADR-0007](../../../decisions/0007-pgvector-implementado-ui-gated-off.md), [ADR-0009](../../../decisions/0009-anonimato-como-regla-de-presentacion.md)
