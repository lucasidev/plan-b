...
| O03 | CONEAU no cubre pregrado: para una tecnicatura no hay acreditación, hay validez nacional (SIPES). El campo cambia de nombre con el nivel (F05). | Tesis línea 75 | Pendiente: el ADR propuesto lo modela como dos campos |
| O04 | La nómina docente de UNT no trae la condición del cargo (regular o interino); la de UTN no trae ni cargo ni regional. La proporción de interinos que pide SC-005 no está publicada. | SC-005 | Pendiente: pedido por Ley 27.275 o el campo queda "no publicado" |
...
| O07 | SIPES y la Guía SIU son aplicaciones Toba con sesión y ventana emergente: automatizarlas es frágil (token por pedido, paginado por evento, ventana que carga por JavaScript), aunque las dos se leyeron enteras en este relevamiento. | Sofía, catálogo | Pendiente: la carga es a mano en R6; automatizar solo si el catálogo crece a más de una ciudad |
| O08 | La AGN tiene API JSON:API y el buscador de la web es un cliente de esa API: el checklist "auditada" se puede automatizar. | SC-005 | Pendiente: R6 decide si entra |
...
| O10 | El sitio de UTN Facultad Regional Tucumán no respondió durante todo el relevamiento y el de las ordenanzas del Consejo Superior rechazó la conexión: el "sitio institucional" como fuente tiene la fragilidad de cada sitio, y la ficha necesita decir la fecha en que se pudo leer. | ADR-0085 | Pendiente: el estado del dato lleva fecha de relevamiento (ADR propuesto) |
| O11 | Los anuarios 2020 y 2021 repiten para UNSTA los valores de 2019 (6393 estudiantes, 1877 nuevos inscriptos, 310 egresados): la institución no informó y la SPU arrastró el dato. Un proxy calculado sobre esas filas miente. | Método | Pendiente: la regla del proxy lo marca como sesgo y la ficha lo dice |
