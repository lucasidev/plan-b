...
|---|---|---|---|
| L01 | El gate no dice el motivo: al llegar a Ingresar desde "Reseñala", la pantalla es la genérica ("Entrá a tu cuenta") y no dice que es para reseñar esa cursada. | US-229 | Resuelto: `?from=` arma el motivo arriba del form ("Para reseñar una cursada, necesitás una cuenta.") cuando viene de una acción reconocida (R6, tarea 12, [#492](https://github.com/lucasidev/plan-b/issues/492)) |
...
| L03 | Después de verificar el mail y entrar, aterriza en Inicio y no vuelve a la reseña que había empezado. | US-229 | Resuelto: el `from` sanitizado viaja de Ingresar a Registro y de vuelta, para aterrizar en la acción que lo disparó (R6, tarea 12, [#492](https://github.com/lucasidev/plan-b/issues/492)) |
| L04 | El contrato antes de enviar no dice el estado del piso de esa cátedra ("junta N: con M más se publica"): dice que se suma al total, que nunca se muestra una reseña sola y que nadie de la facultad accede a quién respondió. | US-159 | Resuelto: el contrato del paso 6 agrega la frase del piso de la cátedra elegida (R6, tarea 13, [#493](https://github.com/lucasidev/plan-b/issues/493)) |
| L05 | Mis aportes muestra cómo terminó y cuántas preguntas contestó, pero no la opción que eligió en cada frase ni las voces que suma ahora. | US-162 | Resuelto: cada frase respondida muestra la opción elegida y las voces que suma ahora ("ahora 22 de 42 voces", R6, tarea 14, [#494](https://github.com/lucasidev/plan-b/issues/494)) |
| L06 | La reseña duplicada del mismo período se detecta y se avisa, pero el botón de enviar sigue habilitado: la persona puede enviar igual y recién después descubre que no contó. | US-163 | Resuelto: el botón de enviar queda deshabilitado cuando la cursada elegida ya se rechazó por duplicada (R6, tarea 13, [#493](https://github.com/lucasidev/plan-b/issues/493)) |
...
| L08 | Tras confirmar el borrado, la ficha de Ruiz siguió mostrando el mismo conteo. No se pudo saber si el borrado no ocurrió (el click de confirmar es un botón de acción, la clase de #477) o si ocurrió y el conteo no bajó: la spec leyó la ficha sin esperar a que el aporte desapareciera de Mis aportes. | US-165 | Resuelto en el producto (R6, tarea 15): en el [recorrido del 2026-09-13](2026-09-13-lucia-walk.md), paso 8, Borrar y "Sí, borrarla" sacan la fila de Mis aportes y la ficha de Ruiz baja de 8 a 7 en el acto; lo que sigue sin llegar es el click de confirmar de la spec, no el borrado. La clase quedó confirmada por [#491](https://github.com/lucasidev/plan-b/issues/491): Corregir y Borrar en Mis aportes navegaban con el mismo `router.refresh()` sin fallback duro que #491 midió fallando bajo carga. Cerrado el 2026-09-10: Corregir pasa a un reload real (mismo remedio que Mi perfil, `lib/reload-after-mutation.ts`); Borrar se queda con `router.refresh()` porque ahí la tarjeta desaparece por un estado local que no depende de ese commit (razón en el propio código) |
