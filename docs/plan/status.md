...
| [#416](https://github.com/lucasidev/plan-b/issues/416) · Q10 | Las nueve rutas sin E2E: docentes del backoffice y universidad nueva con su spec; `design-check`, `maintenance` y `offline` con un smoke; `teacher-claim` y `verify-teacher` son faltantes al revés, código del modelo anterior sin story vigente, y su destino (borrar o probar) lo decide Lucas antes de escribirles un test. **Decidido el 2026-09-03**: se borraron, con su feature y los cuatro endpoints del claim en identity; `TeacherProfile` queda y su destino es decisión aparte. Faltan los specs de las siete rutas vivas | 2,5 |
