...
| # | Tarea | Pts | Estado |
|---|---|---|---|
| [#517](https://github.com/lucasidev/plan-b/issues/517) · 1 | **El refresh silencioso** (L11, US-229): `middleware.ts` renueva el access token con la cookie de refresh contra `/api/identity/refresh` y rota las cookies en la respuesta; al mergear se borra `JWT__AccessTokenMinutes` de la Application API del stage. Listo cuando: una sesión de una hora sobre el stage no pide entrar de nuevo, sin la variable de Dokploy, y un test cubre el vencimiento con refresh vigente y sin él. | 5 | Hecho: la cookie de refresh en `Path=/` y `middleware.ts` renovando en las rutas con cuenta, con deadline, sin renovar en prefetches y borrando la cookie vieja al cerrar sesión ([ADR-0095](../decisions/0095-the-session-renews-itself-in-the-middleware.md)); `session-renews.spec.ts` en verde contra el stack local (renueva y rota las dos cookies; con el refresh revocado va a Ingresar). Al desplegar, se borra `JWT__AccessTokenMinutes` de la Application API del stage |
