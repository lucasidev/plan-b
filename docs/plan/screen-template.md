...
- **Toda pantalla tiene una épica dueña**, y es la que la hace existir. Cuando otra épica le aporta una acción (corregir un dato, responder), lo dice en su README y la ficha la nombra en "Qué stories resuelve".
- **Los invariantes del producto valen acá también**: el rojo solo marca la opción negativa de un dato publicado, todo conteo viaja con sus voces, y ningún dato se promedia con otro ([ADR-0083](../decisions/0083-the-ficha-publishes-counts-not-scores.md)), más los períodos nunca codificados ([ADR-0051](../decisions/0051-academic-vocabulary-with-a-canonical-representation-in-the-ui.md)). Una pantalla que los rompe está mal aunque respete este contrato.
