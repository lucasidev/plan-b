...
6. **Deploy**, confirmar, y esperar: la pestaña Deployments muestra la corrida y *View* abre su log. Bajar las imágenes y levantarlas lleva unos dos minutos y termina en `Docker Compose Deployed`. El contenedor `migrate` tiene que terminar en exit 0 antes de que `api` arranque; *Healthy* en el `api` significa que escucha y que Postgres y Redis contestan. La base queda migrada y **vacía**: el despliegue no siembra ([ADR-0093](../decisions/0093-the-deploy-migrates-the-schema-and-never-seeds-data.md)).
7. **Sembrar, una vez**: el verbo `seed-db` de la misma imagen del `api`, en la red `internal` del compose y con las mismas variables de conexión que `api` (comandos exactos en [`runbook.md`](runbook.md), caso 7). Carga personas, catálogo académico, catálogo de frases y corpus; la última línea es `CorpusSeeder: inserted N reviews`. De ahí en más los datos del stage persisten y no se rehacen en cada deploy: cargar un catálogo nuevo es correr esto de nuevo, a mano.
8. **Verificar**: `https://planb.olisar.com.ar/health` (el rewrite del frontend lo lleva al `api`) devuelve `{"status":"ok",...}`; `https://planb.olisar.com.ar/` muestra la entrada con el corpus; `https://mail.olisar.com.ar/` pide el usuario y la password de `MAILPIT_UI_AUTH`.
9. **Reset**: desde la terminal del servidor, `docker compose -p <App Name> down -v` con el App Name que muestra la cabecera del servicio, y volver a Deploy. El compose deja la base migrada y vacía, así que volver a cero son dos movimientos: el reset y después el paso 7.
...
**Decidido y sin aplicar** (2026-09-10, [ADR-0093](../decisions/0093-the-deploy-migrates-the-schema-and-never-seeds-data.md)): esta sección ya describe el despliegue que migra y no siembra, y `docker-compose.stage.yml` todavía trae el servicio `seed` con `api` esperándolo. Hasta que el servicio salga del compose, un deploy sigue sembrando y una falla del seed sigue impidiendo que `api` arranque, que es el incidente que originó la decisión.

**Sin verificar**: el reset por terminal (`down -v` y Deploy) contra el despliegue real, la siembra a mano contra el stage (paso 7 del guion de clics), el tramo con cuenta del recorrido (verificar desde Mailpit, reseñar, el backoffice), y la espera del sha en `/health` desde el workflow, que verifica el primer merge después de este cambio.
...
8. **Volver a cero**: el reset del paso 9 del guion de clics, seguido de la siembra del paso 7, deja el stage exactamente como al principio, con el mismo corpus y sin la cuenta de prueba.
