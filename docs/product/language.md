...
| **datos de prueba** | Datos sembrados en la DB para probar y mostrar la aplicación real funcionando (desarrollo, tests, defensa ante el tribunal). Los consumen los endpoints reales. Se siembran las personas, el catálogo académico y el **corpus de demostración**: en Development lo gatea `PLANB_SEED_CORPUS`; en el stage corre como paso de deploy (`seed-db`); en producción no existe el verbo que pueda dispararlo ([ADR-0091](../decisions/0091-the-stage-runs-as-production-and-seeding-is-a-deploy-step.md)). |
