...
| **Frase** | Una oración predefinida ("Es dura de verdad", "Hay clases que no se dan", "Nunca te ponen un 10"). No se escribe: se **marca**. Las hay nuestras (las semilla) y destiladas. Cada frase tiene un sujeto y un eje. |
| **Marcar** | Decir "esto me pasó" sobre una frase. Es lo mínimo que hace una reseña. **No se dice "tocar"**. |
| **Sujeto** de una frase | A qué ficha va el dato: la materia, la cátedra o la institución (las fichas reales; la carrera se deriva y no es sujeto). El mostrador y el centro de estudiantes no son sujetos: son **temas** de la vivencia institucional. |
| **Eje** de una frase | Su atribución, escrita en la redacción: **exigencia** (la carrera siendo así) o **gestión** (alguien fallando). De qué parte de la vivencia habla lo dice el **tema**, no el eje. |
...
| **Encogimiento** | Cómo se corrige una proporción cuando la sostienen pocas voces: se publica el límite inferior del intervalo de Wilson, que sube solo a medida que crece el corpus. Cuatro de cuatro no es 100%. Con nombre y fórmula publicada en Método. |
| **Los dos ejes** | Exigencia y gestión, los dos valores posibles de la atribución de una frase. Nunca se mezclan (lo uno es información, lo otro alarma) y ninguno produce un número propio: no hay puntaje ni resumen por eje ([ADR-0078](../decisions/0078-the-questionnaire-collects-in-pairs-the-ficha-reports-by-theme.md)). |
| **Atribución** | De qué lado cae una frase: "la carrera siendo dura" (eje exigencia) o "alguien fallando" (eje gestión). **La decide el eje**, nunca el sujeto ni la persona ([ADR-0065](../decisions/0065-attribution-is-the-axis-not-a-split.md)). Viaja **en cada hecho**, nunca como cabecera dual ni como split; cómo se muestra lo decide la ficha, con la alarma visual reservada a gestión ([ADR-0078](../decisions/0078-the-questionnaire-collects-in-pairs-the-ficha-reports-by-theme.md)): un número de atribución global no dice qué arreglar ni qué elegir, y un split depende del catálogo, no de la gente. |
| **Frase semilla** | Una de las frases que ofrecemos nosotros para arrancar (el catálogo vive en [phrases.md](phrases.md): las del canvas más las que la tesis pide). Se distingue de la **frase destilada**, que salió de los comentarios de muchos y las voces validaron. |
| **Tema** | La tercera coordenada de una frase, junto al sujeto y al eje: qué parte de la vivencia nombra (enseñanza, evaluación, cumplimiento, trato, carga y vida, economía, infraestructura, trámites; la lista la sostiene la curaduría, con un presupuesto de 6 a 10 hechos por tema). Organiza el cuestionario y el reporte de la ficha; no atribuye: dentro de cada tema, el eje sigue decidiendo quién carga ([ADR-0078](../decisions/0078-the-questionnaire-collects-in-pairs-the-ficha-reports-by-theme.md)). |
| **Reputación** | Lo que la ficha muestra de un sujeto: sus hechos sostenidos por tema, con sus voces, su serie y su réplica. **Nunca un número**: no hay score, ni índice, ni promedio; los "reputation systems" que puntúan son exactamente lo que esto no es ([ADR-0078](../decisions/0078-the-questionnaire-collects-in-pairs-the-ficha-reports-by-theme.md)). |
| **Prevalencia por tema** | El indicador de sumario de la ficha: cuántas personas marcaron al menos un hecho de sentido negativo en un tema, y aparte cuántas de sentido positivo, contra el mismo denominador de siempre; una persona cuenta una vez por lado. Es la *positivity* del NSS dicha en criollo, con definición publicada en el Método ([ADR-0078](../decisions/0078-the-questionnaire-collects-in-pairs-the-ficha-reports-by-theme.md)). |
...
| **Cobertura** | Cuántas de las materias canónicas de la carrera tienen voces, sobre el total ("22 de 40 materias"); cuando coexisten dos planes, la materia canónica los une y el denominador es uno solo (D04). Es distinto de las voces: 850 voces en 3 materias es mucha voz y poca cobertura. Viaja con todo dato derivado, y cada frase derivada dice en cuántas materias aparece. |
| **Gate de cobertura** | La condición para publicar la cabecera derivada de una carrera o institución: más de la mitad de las materias canónicas de la carrera con voces. Debajo, la ficha dice que todavía no derivamos y se lee materia por materia. No es un piso: las listas de frases se publican desde la primera voz. |
...
| **Aprobación** | Aprobé sobre aprobé más desaprobé, por materia y período; afuera dejé, regular y sigo (la definición de [ADR-0047](../decisions/0047-public-pass-rate-from-private-enrollment-history.md), desde lo declarado). **Abandono de cursada**: dejé sobre todos los que terminaron de alguna forma. |
...
| **Marcar el plan** | Señalar en tu plan qué materias te faltan o considerás, para que la co-cursada se filtre a tu caso. Es **preferencia privada, no dato**: no se recaba, no se publica, no entra a ningún agregado ([ADR-0069](../decisions/0069-the-marked-plan-is-a-private-preference-not-a-fact.md)). Lo que reseñaste con cómo terminó viene marcado solo, porque eso sí es un hecho. **No se dice "corral"** (era jerga del mapa). |
| **Clases sin dar** | Cuántas clases no se dieron en una cursada, declarado solo por quien marcó que hubo. Es la única cantidad que se publica que no es una frase: por cátedra y período, mediana y rango con sus voces ("4, entre 2 y 8, 12 voces"), sin piso; nunca un valor único. |
...
| **Carrera canónica** | El nombre bajo el que el catálogo declara que dos ofertas de distintas instituciones son la misma carrera, para compararlas en Dónde estudiarla. Lo decide el equipo, no el parecido del nombre. |
| **Nombre de pantalla** | Una pantalla se nombra por lo que dice arriba, en español, con mayúscula inicial y sin backticks: Mi carrera, Dónde estudiarla, Método, Reseñar, Ficha de cátedra, Mi situación. Es el mismo nombre en el canvas y en los docs. **Los backticks son código**: la URL va en inglés y con slug (`/my-career`, `/reviews/write`) y se fija cuando la pantalla entra a sprint. |
| **Dónde estudiarla** | La misma carrera canónica en varias instituciones, lado a lado, dato por dato, sin compuesto, sin ganador y sin ordenar por valor. Quien quiere ordenar baja el CSV. |
...
| **réplica firmada** | Toda réplica la publica una persona con su nombre y su cargo, nunca una entidad: no existe "responde la UNSTA", responde alguien de la UNSTA ([ADR-0068](../decisions/0068-comment-publishes-as-testimony-below-the-phrases.md), [ADR-0073](../decisions/0073-the-team-verifies-who-replies-against-its-own-catalog.md)). | |
...
Los actores del producto nuevo son las [personas](personas.md) y la lista cerrada de roles de los [requisitos](README.md): quien lee (sin cuenta), quien reseña y vota (con cuenta), el docente y la institución (réplica con identidad verificada), y el equipo (catálogo, frases, moderación, verificación, administración). Qué puede y qué no puede cada uno lo dice cada requisito; la tabla de actores de la versión anterior está en el [ático](../history/domain-v1/ubiquitous-language-v1.md).
