...
| **Frase** | La unidad del catálogo: una pregunta con opciones cerradas, en frecuencias gruesas que la memoria puede responder ("¿Contestaba las preguntas en clase?"). Tiene **código estable**: el texto puede afinarse; si cambia el significado, es una frase nueva y la serie se corta ([ADR-0082](../decisions/0082-the-review-captures-the-cursada-in-three-layers.md)). |
| **Opción** | Cada respuesta posible de una frase ("Siempre / A veces / Casi nunca / Nadie preguntaba"). Cada una tiene su valencia; la negativa es la única que carga el rojo en la ficha. |
| **Responder** | Elegir una opción de una frase. Saltear es explícito, legítimo, y no cuenta en el denominador de esa frase. |
| **Instrumento** | Una versión del cuestionario: qué frases ofrece, en qué orden, con qué condiciones. Se versiona con vigencias y cada reseña queda atada a la versión con la que respondió. |
...
| **Curaduría** | El trabajo del equipo sobre el campo libre: leer, **destilar** frases nuevas para la versión siguiente del instrumento, y escribir **notas editoriales**. También cura el catálogo académico y el relevamiento oficial. |
...
| **Frase semilla / destilada** | La semilla la escribimos nosotros para arrancar; la destilada sale del campo libre de muchos y entra al instrumento como versión nueva. |
...
| **Moda** | La opción literal más votada de una frase. Es la síntesis publicada, como badge que repite esa opción con su porcentaje ("Casi nunca · 59 %"): nunca un promedio ni una etiqueta inventada. |
| **Distribución** | El conteo completo de una frase, opción por opción, como barra segmentada. Es la única visualización de una frase: no hay promedios. |
| **Convergencia** | La fama del sujeto: cuando varias frases distintas apuntan al mismo lado ("no salían entendiendo" + "no podían preguntar" + "no contestaba"), la ficha lo dice arriba, predicado del sujeto. Tres frases convergentes valen más que quinientas marcas en una. |
...
| **Serie** | La misma frase por el período en que pasó, nunca por cuándo se reseñó. Si la frase cambió de código, la serie declara el corte y los tramos no se comparan a través. |
...
| **archivar** | Sacar del catálogo algo que ya no se ofrece, sin borrarlo (`is_active = false`). Aplica a University, Career, Subject y Teacher; CareerPlan usa su propio `status` porque deprecar no es lo mismo que archivar. Lo archivado desaparece de las lecturas públicas y sigue existiendo para todo lo que ya lo referencia: una cursada vieja, una reseña, un plan ([ADR-0057](../decisions/0057-soft-delete-when-something-hangs-off-it.md)). |
...
| **ítem** | Como nombre del concepto en el producto | En el producto se llama **frase**. `Item` es solo el identificador en el código, donde todo va en inglés; en la UI, en los docs y en la conversación es "frase". |
...
| **corregir** | "Cambiar lo que dije en una reseña" | Eso se dice **editar**. Corregir queda para un dato duro de una ficha ([US-189](student/care-for-what-is-published/stories/US-189-correct-a-hard-fact-inline/README.md)), para la redacción de una frase en la curaduría (US-198) y para la corrección estadística que Método explica. |
