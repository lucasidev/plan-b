...
| **Atribución** | De qué lado cae una frase: "la carrera siendo dura" (eje exigencia) o "alguien fallando" (eje gestión). **La decide el eje**, nunca el sujeto ni la persona ([ADR-0065](../decisions/0065-attribution-is-the-axis-not-a-split.md)). Viaja como **etiqueta de cada hecho** en la ficha ("así es la carrera" / "alguien falla"; la alarma visual sigue reservada al segundo caso), nunca como cabecera dual ni como split ([ADR-0078](../decisions/0078-the-questionnaire-collects-in-pairs-the-ficha-reports-by-theme.md)): un número de atribución global no dice qué arreglar ni qué elegir, y un split depende del catálogo, no de la gente. |
