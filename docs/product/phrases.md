...
Las preguntas que el producto pregunta al reseñar una cursada, con sus opciones, y los instrumentos que las agrupan. Es **contenido editorial nuestro** y la decisión más visible del producto ([ADR-0082](../decisions/0082-the-review-captures-the-cursada-in-three-layers.md)): se pregunta conducta observable y vivencia en frecuencias gruesas, y la ficha publica sus conteos ([ADR-0083](../decisions/0083-the-ficha-publishes-counts-not-scores.md)). Se edita en un solo lugar (el backoffice: US-198); a estas preguntas semilla se suman las **destiladas** del campo libre, aprobadas antes de ofrecerse (US-199). El archivo conserva su nombre histórico (`phrases.md`) hasta la propagación completa.

> **Estado (2026-08-25)**: catálogo rehecho al modelo de tres capas y aprobado en boceto. Cada pregunta tendrá su código estable al cargarse a la base (issue #357); si una pregunta cambia de significado, es un código nuevo y su serie se corta.

## Las reglas

1. **Cada pregunta nace de una inquietud real**: algo que un estudiante quiere saber antes de meterse, o quiere decir al salir. Si no traza a eso, no entra.
...
5. **Saltear siempre vale**: quien saltea no cuenta en el denominador de esa pregunta. Nada es obligatorio.
...
| Pregunta | Opciones (la negativa, en negrita) |
|---|---|
...
Uno solo, al final: **"¿Algo que no te preguntamos y deberíamos?"**. La pantalla dice para qué sirve y que no se publica. Alimenta a la curaduría: destilación de preguntas nuevas y notas editoriales sin nombres ([ADR-0084](../decisions/0084-free-text-feeds-curation-and-is-never-published.md)).
...
- **El maltrato y el acoso**: cómo entra al instrumento (pregunta directa, campo propio, o solo por el campo libre hacia la curaduría) sigue siendo decisión abierta de producto. Hasta decidirse, el campo libre recibe lo que alguien quiera decir, y no se publica.
