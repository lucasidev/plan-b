...
**E2.** Dado que "Toman lo que no dieron" es una frase destilada del campo libre, aprobada por quien cura el catálogo, en la capa de conducta observable
Cuando se muestra en el bloque de conducta observable de una ficha
Entonces aparece marcada como "síntesis" al lado de su moda y su distribución, distinta de una frase semilla como "¿Se dictaron las clases?", que no lleva esa marca.
...
Entonces esa frase no se ofrece para responder y no aparece en ninguna ficha ni en el CSV: recién se ofrece, marcada como destilada, después de que quien cura el catálogo la apruebe con su capa (US-199).
...
- Quien cura el catálogo corrige la capa de "¿Se dictaron las clases?": la corrección reprocesa todas las fichas que usan esa frase, y Método declara el cambio con autor y fecha (US-198). **Falta decidir**: cómo se versiona el catálogo para que una cita puntual de Rocío sea exactamente reproducible más allá de declarar la fecha de lectura.
