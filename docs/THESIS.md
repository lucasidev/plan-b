...
**Vigente desde**: 2026-08-16. **Reescrita el 2026-09-13**: el objeto, el problema y el fin último dicen lo que plan-b es para quien lo usa; las decisiones que lo gobiernan viven en las ADR que cada sección cita ([ADR-0082](decisions/0082-the-review-captures-the-cursada-in-three-layers.md) a [ADR-0085](decisions/0085-three-instruments-and-official-data.md), [ADR-0096](decisions/0096-explorar-interprets-official-data-by-institution-and-career.md)).

Este documento es la tesis del producto: lo que plan-b es, lo que no hace, y la posición que toma. Todo lo demás del repo se lee contra esto.

---

## Qué es

plan-b existe para que la comunidad universitaria ponga en común, entre todos, la información más valiosa que tiene y que ninguna institución puede producir por ella: lo que cada uno sabe por haber cursado. Cómo fue la cursada, cómo fue pasar por esa universidad, qué pasó de verdad. No sale de la institución ni de su marketing: sale de quienes lo vivieron, cruda y real, y puesta en común deja de ser anécdota para volverse un dato.

No es un buscador de carreras, ni un ranking, ni una app de gestión académica. Es el lugar donde lo que hoy es un comentario suelto y desmentible se vuelve un dato que aguanta una discusión.

## El problema

Quien quiere estudiar una carrera encuentra dos clases de información sobre cómo es estudiarla de verdad: la que carga la institución, hecha de marketing, y la que circula suelta, poco confiable. La que vale, la de quienes ya cursaron, existe: vive en grupos de WhatsApp y en los pasillos de la facultad. Pero nadie la recolecta, nadie la conserva y nadie la trabaja para sacarle lo que enseña.

Un alumno diciendo "no dieron las clases" es una anécdota. Cuarenta diciéndolo es un hecho. Lo único que separa una cosa de la otra es que están dispersos y en silencio.
...
5. **Lo destilado**: del campo libre de muchos salen preguntas nuevas, versionadas en el catálogo. Es un dato derivado, no pedido.
...
1. **La unidad de publicación es el conteo de una pregunta**: la moda como síntesis (la opción literal más votada, con su porcentaje) y la distribución completa por opción. Cada pregunta es su propio dato con su propio denominador (quienes la respondieron); nada se promedia con nada.
2. **La fama es la convergencia.** Lo primero que la ficha dice del sujeto son los hechos donde varias preguntas distintas apuntan al mismo lado, predicados de él ("acá no se aprende preguntando"), con el sustento como metadato. Tres preguntas convergentes valen más que quinientas marcas en una.
3. **Comparar es solo contra los pares directos**: una cátedra contra las otras cátedras de su misma materia, donde el sesgo de quién reseña pega parejo y se cancela en la diferencia. El contraste se publica solo si los intervalos no se tocan (Wilson, como maquinaria interna, no como número publicado); sin señal o sin base comparable, silencio. Jamás se cruzan señales de reseñas entre instituciones. Lo oficial sí se compara entre instituciones y carreras, dato por dato y con la fuente: cuál eligen más, cuál egresa más, dónde más alumnos avanzan, cuál acreditó la entidad auditora. Se dice quién va primero en cada dato; no se suman en un ganador. Donde el alumno elige cátedra, la comparación es de a dos, pregunta por pregunta, con la finalización de cada una.
4. **La tasa de finalización se publica agregada** ("de cada 10 que la cursan, llegan 4"): es un resultado de la cátedra y de la universidad, que tiene que luchar por que se reciban. El desenlace individual no se muestra jamás. Y se cruza con lo que dijeron: entre quienes la dejaron, qué preguntas cargaron ("el 80 % casi nunca salía entendiendo"), con el mismo piso y sin exponer a nadie.
...
7. **Lo que no se reseña se deriva, condicionado a cobertura**: la materia muestra la dispersión entre sus cátedras ("depende de cuál te toque" es el dato); la carrera muestra estructura (qué materia frena a cuántas, en qué año y en qué materias se corta la cursada, cuántos cuatrimestres tarda de verdad el primer año) y cuánto del plan está medido. La carrera de la que nadie habló no es impecable: es desconocida, y la ficha lo dice.
8. **La institución no tiene puntaje**: su ficha es su identidad relevada (cuántos la eligen, cuántos egresan, cuántos avanzan), su plantel navegable, su transparencia relevada a fuente pública (con fecha y fuentes), las notas de curaduría, su cobertura, y cómo es tratar con ella: los conteos del instrumento administrativo (cuánto tarda un trámite, si la beca llegó). Explorar dice cuál va primero en cada uno de esos datos, con la fuente al lado.
...
10. **La curaduría publica notas sin nombres** a nivel carrera, institución o materia, donde el patrón se repite, con procedencia declarada ("leída de comentarios que no se publican") y fecha ([ADR-0084](decisions/0084-free-text-feeds-curation-and-is-never-published.md)).
11. **El instrumento se versiona y la serie declara sus cortes**: si una pregunta cambia de significado, cambia de código y la serie no se compara a través. La lista se reprocesa a medida que entran reseñas, y se dice. Y la ficha dice la tendencia con palabras: si la cátedra mejoró, empeoró o sigue igual respecto del período anterior.
12. **El método entero es público**, con el catálogo de preguntas, las reglas de publicación y el piso; y los datos se bajan como se publican, agregados: no existe un crudo que tenga más que la ficha. Por carrera o por institución hay una hoja para llevar: los conteos con sus voces y sus fuentes, para una reunión.
13. **Las que están por publicarse**: quien tiene cuenta ve las cátedras de su carrera a una o dos reseñas del piso, y que su reseña las publica. Es la palanca de la comunidad, y la ficha la muestra.
...
- Leer no pide cuenta; producir sí. La puerta llega en la acción (reseñar, editar), nunca antes de leer: si el muro está antes del valor, no hay corpus.
- Conteos con sus voces, nunca puntajes: la moda literal con su distribución, y ningún número que resuma varios datos en uno ([ADR-0083](decisions/0083-the-ficha-publishes-counts-not-scores.md)). Interpretar sí: lo que un dato dice, con su fuente al lado ([ADR-0096](decisions/0096-explorar-interprets-official-data-by-institution-and-career.md)).
- El catálogo lo carga el equipo, completo, y lo oficial viene con fuente y fecha: lo que se pone en común es la vivencia, nunca el dato base ([ADR-0085](decisions/0085-three-instruments-and-official-data.md)).
