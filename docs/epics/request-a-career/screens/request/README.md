...
**Ana** (su facultad no está y sospecha del vacío: si no se explica no vuelve; si se explica, empuja para que se cargue). Del otro lado, **Sofía** carga por esta cola ([Sostener el catálogo](../../../sustain-the-catalog/README.md)). El flujo entero: [`flow.md`](../../flow.md).

## Qué stories resuelve

O2-2 (dueña: el pedido se manda con el mail y nada más, y entra a la cola cuando ese mail se confirma por link), O2-4 (el aviso cuando la carguen, y si te registrás, el pedido precarga institución y carrera), O2-1 (el estado del vacío que trae hasta acá: "no la cargamos todavía" no es un cero). La letra de cada una: [README de la épica](../../README.md#stories).

## Qué muestra

Un campo de institución, uno de carrera (cómo se elige o se escribe queda abierto) y el mail. Debajo, el aviso: "te mandamos un link para confirmar; el pedido cuenta cuando lo confirmás; un mail cuenta una vez por carrera" (O2-2, D03).

**Estados**:
- **Mail enviado**: pide confirmar desde ese mail; si no confirmás, el pedido no entra a la cola ni cuenta como reclamo.
- **Confirmado**: "tu pedido cuenta: sos el 23° que la pide", con el link a [La cola](../queue/README.md).
...
Al mail de confirmación (el link) y de ahí al estado "el pedido cuenta"; a la ficha de la carrera cuando ya estaba cargada; a [La cola](../queue/README.md). Llega desde el vacío explicado en Explorar, Buscar y la Ficha de carrera (O2-1).

## Decisiones que aplica

D03 ([registro del 17](../../../../reviews/2026-08-17-catalog-propagation.md): el pedido confirma el mail por link, igual que el reporte; un mail cuenta una vez por carrera), los [tres planos](../../../../design/product-map.md#los-tres-planos) del mapa de producto (el pedido es el único plano donde alguien sin cobertura tiene lugar; cuánta gente reclama dice dónde mirar).
