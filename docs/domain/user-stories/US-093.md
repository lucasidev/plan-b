...
- [ ] `UNIQUE(subject_id, term_id, name)`, sobre todas las filas y no solo las activas: archivar la comisión "A" no libera ese nombre, y la forma de volver a tener una "A" es reactivar la archivada, no crear una segunda ([ADR-0057](../../decisions/0057-borrado-logico-cuando-hay-algo-colgando.md)). Coherencia de cadencia (`subject.term_kind = term.kind`) y de universidad (subject, term, teacher de la misma).
...
- Mockup admin canvas (sección ②): ![Comisiones · oferta 2026·1c](../../history/design-v1/reference/screenshots/admin-datos-adm-com-list.png) (`canvas-mocks/admin-screens-2.jsx::AdmComisiones`)
