# ADR-0012: Review editing is blocked while a report is moderating it
...
- Relacionado con [ADR-0005](0005-review-anchored-to-the-enrollment-record.md) (anclaje a enrollment, `UNIQUE(enrollment_id)`).
...
**Eso es la alternativa C de este mismo ADR** ("el alumno puede editar pero no si hay reports open"), con un refinamiento: el candado no cuelga de "hay reports abiertos" sino de la razón por la que la reseña entró a cuarentena. Esa razón pasa de un bool a `under_review_reason` con tres valores (`content_filter`, `reports`, `enrollment_changed`, el último de [ADR-0032](0032-destructive-enrollment-edit-invalidates-its-review.md)). De los dos motivos por los que C se descartó en abril, uno no aplica y el otro se paga:
