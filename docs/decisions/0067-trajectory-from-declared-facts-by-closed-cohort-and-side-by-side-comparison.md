...
La tesis promete cuatro cosas que no salen de frases sino de trayectoria: cuánto tarda la gente de verdad, dónde se cae la mayoría, qué se llevó junto y cuántos dejaron una ([THESIS.md](../THESIS.md), decisión 4). Las stories las reparten entre Valentina (O1-1: nominal, real y de cuántos egresados sale), Silvia (O1-7: duración real y cuántos se reciben, sin vocabulario), Matías (O3-1: por par de materias, cuántos las llevaron juntas y cuántos dejaron una), Diego (O4-8: en qué punto del plan se cae la mayoría), Claudia y la institución (O7-7: la serie, "si mejoré desde que lo publicaron"), y las comparaciones de `donde` (O1-2: la misma carrera en varias instituciones; O7-3: "en qué estoy peor que la de al lado").
...
6. **`donde`: la misma carrera, lado a lado, dato por dato.** Qué ofertas son "la misma carrera" lo decide el catálogo con una **carrera canónica** curada por nosotros ("Ingeniería en Sistemas" y "en Sistemas de Información" son una o no lo son porque lo decidimos, no por el nombre). Se muestran nominal, real, brecha, egreso de cohortes cerradas, las dos cabeceras (con su gate: "todavía no derivamos" si no llegó), la cobertura y las listas por eje. **Sin compuesto, sin ganador y sin ordenar por valor**: alfabético o por voces; el que quiere ordenar baja el CSV. Es lo único que mantiene "no es un ranking" verdadero. O7-3 se cumple así: frase por frase, no con un puesto.
...
**E. `donde` con un compuesto o ordenado por valor.** Es un ranking con otro nombre, y el producto dice que no lo es. Descartada; el CSV deja ordenar a quien quiera.
...
- **El catálogo necesita la carrera canónica** para que `donde` sepa qué compara. Es trabajo del equipo, como todo el catálogo.
