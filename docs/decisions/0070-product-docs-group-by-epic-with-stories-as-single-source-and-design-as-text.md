...
3. **La pantalla compartida es el inventario** (el sitemap): `docs/design/screens/<pantalla>/` con `README.md` (la ficha: quién la usa, qué stories resuelve, qué muestra, estados, acciones, adónde va, slug) y `boceto.html`. Una pantalla la componen varias épicas, como una page compone features; la ficha lista cuáles.
...
- **Las épicas quedan resueltas por construcción**: son las carpetas; `docs/domain/epics.md` deja de existir y `docs/epics/README.md` es el índice. La primera épica escrita completa es Reseñar (`docs/epics/resenar/`).
