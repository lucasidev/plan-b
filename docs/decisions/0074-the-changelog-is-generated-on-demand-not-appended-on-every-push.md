...
- **Una sola sección, `[Unreleased]`, desde el primer día.** Nunca se cortó una versión, porque [ADR-0089](0089-the-stage-follows-main-and-production-is-promoted-from-a-release.md) fija que no hay versiones ni Releases hasta que haya producción.
...
3. **El disparador es el primer Release a producción ([ADR-0089](0089-the-stage-follows-main-and-production-is-promoted-from-a-release.md))**. Antes de eso, si hace falta un snapshot (una entrega, una presentación), se genera con el mismo comando.
4. **La regla de no editarlo a mano sigue**, y ahora por una razón más simple: no se edita porque se genera.

**Lo que no cambia**: la política de versionado de [ADR-0089](0089-the-stage-follows-main-and-production-is-promoted-from-a-release.md) queda intacta (sin versiones, tags semver ni Releases hasta producción). Esto reabre cómo se produce el changelog, no cuándo se versiona.
...
**C. `semantic-release`.** Automatiza versión, changelog y publish en cada merge. Asume release continuo, que es exactamente lo que ADR-0089 dice que este proyecto no hace: el stage sigue a `main`, pero un Release es una decisión. Descartada.
...
- La automatización por auto-append (lo que esto supersede, ya fuera de la carpeta), [ADR-0089](0089-the-stage-follows-main-and-production-is-promoted-from-a-release.md) (la política de versionado, intacta), [ADR-0026](0026-git-workflow-github-flow-with-rebase.md) (Conventional Commits, que es lo que hace posible generar después).
