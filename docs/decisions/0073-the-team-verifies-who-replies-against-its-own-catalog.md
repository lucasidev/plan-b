...
La respuesta del reseñado es una de las decisiones de la tesis: lo publicado necesita que el evaluado pueda responder. Pero **quién puede responder y cómo se prueba que es quien dice ser nunca se decidió**, y el catálogo lo arrastra sin darse cuenta:

- Las stories de [Responder](../product/reviewed/reply/README.md): cinco con rol "el docente", dos con rol "la institución", una de quien ya aportó. **Las dos de la institución no tienen camino de verificación**: la pantalla [Verificar](../product/student/care-for-what-is-published/screens/SC-022-verify/README.md) tiene dos tarjetas (alumno, docente), no tres.
...
1. **La respuesta la firma una persona.** No existe "responde la UNSTA": responde alguien, con su nombre y su cargo, y así se publica. Esto no es nuevo: el modelo anterior ya publicaba la respuesta con nombre y rol, al lado del testimonio. El testimonio público murió después ([ADR-0084](0084-free-text-feeds-curation-and-is-never-published.md)) y la firma con nombre y cargo quedó igual; acá se cobra su consecuencia. **Verificar autoridad institucional es irresoluble sin colaboración; verificar que una persona es quien dice ser contra un dato que ya tenemos, no.**
...
4. **Verificar es comparar contra ese catálogo, y lo hace el equipo.** Quien pide responder dice quién es; el backoffice compara contra lo que ya está cargado y decide. **No hay auto-servicio ni verificación automática**: hay una persona del equipo resolviendo una cola, con autor y fecha (US-216), como toda acción del backoffice.
5. **Si el catálogo no tiene ese dato, primero se carga.** Un pedido para responder sobre una cátedra sin equipo docente cargado no se rechaza: se convierte en trabajo de catálogo ([Sostener el catálogo](../product/team/sustain-the-catalog/README.md)) y se resuelve cuando el dato está. El hueco es nuestro y se dice, como con cualquier ficha que no existe todavía.
...
8. **La verificación se revalida una vez al año.** Un cargo no es permanente: alguien deja de ser docente o cambia de área, y su respuesta queda publicada firmada con algo que ya no es cierto. Al año, la verificación vence y hay que renovarla; lo ya publicado no se retira, porque era cierto cuando se publicó.
9. **Ante la duda, no se verifica.** El daño no es simétrico: una respuesta falsa firmada con el nombre de una universidad destruye la credibilidad del corpus entero, y la ausencia de respuesta solo deja el canal declarado como vacío. Un rechazo pide motivo y no marca a nadie (US-211): se puede volver a intentar.
...
**D. Autodeclaración con corrección posterior** (responde cualquiera, se corrige si se prueba falso). Barata y rápida, pero invierte el daño: la respuesta falsa ya se publicó con nombre de la universidad, y el desmentido nunca alcanza a quien la leyó. Contradice el punto 7. Descartada.
...
- **[Verificar](../product/student/care-for-what-is-published/screens/SC-022-verify/README.md) pasa de dos caminos a tres**: alumno (señal), docente (permiso para responder) y cargo institucional (permiso para responder por la institución). La ficha y su boceto se rehacen.
...
- **Una respuesta puede quedar esperando a que el catálogo se ponga al día**, y eso es visible: el pedido entra a la cola de catálogo con su demora, como cualquier otro (US-212).
...
- **Qué pasa con la respuesta cuando la verificación vence** y la persona no la renueva: si el canal queda declarado como vacío otra vez, o si el cargo se publica con la fecha en que se verificó.
...
- La respuesta del reseñado con nombre y rol, premisa de este ADR: viene del modelo anterior y su ADR se retiró con él. [ADR-0048](0048-standing-is-opt-in-and-decoupled-from-email.md) (el email institucional retirado), [ADR-0050](0050-backoffice-is-a-cross-cutting-slice-not-a-module.md) (el backoffice como corte transversal), [ADR-0085](0085-three-instruments-and-official-data.md) (cada instrumento tiene su sujeto: de ahí que la institución responda por lo suyo y el docente por lo suyo).
...
- La épica [Responder](../product/reviewed/reply/README.md) y la pantalla [Verificar](../product/student/care-for-what-is-published/screens/SC-022-verify/README.md).
