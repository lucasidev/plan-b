# ADR-0035: Environment configuration
...
| Stage (Dokploy, sigue a `main`) | `Production` | `appsettings.json` ← env vars de Dokploy; `migrate-db` corre como Application run-once antes de API y `seed-db` lo corre una persona, nunca al arrancar ni durante el deploy ([ADR-0093](0093-the-deploy-migrates-the-schema-and-never-seeds-data.md), [ADR-0094](0094-dokploy-applications-are-the-deployment-lifecycle-unit.md)) |
