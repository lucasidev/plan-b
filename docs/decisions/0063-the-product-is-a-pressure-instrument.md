...
Con la convención de entonces, el ADR superado marcaba su `Estado` y el archivo nunca se borraba. La convención cambió después: la carpeta guarda solo lo vigente y los ADRs superados se retiraron de ella, así que acá se nombran en prosa, sin número.

- **Intacto y extendido**: [ADR-0048](0048-standing-is-opt-in-and-decoupled-from-email.md). La tesis resolvió la verificación por el lado que 0048 ya había elegido: aportar pide cuenta, no constancia (exigirla a todos pondría el muro antes del valor), y la condición probada de alumno es una señal opcional que viaja con el dato en vez de habilitar el acto. La oficialización por evidencia (nunca por email institucional) pasa de badge de perfil a semilla del mecanismo de esa señal (US-091/US-092); el cómo exacto (desglose junto al número, umbrales de los datos más finos) es diseño del sistema de frases.
- **Superados enteros por este ADR**: las reseñas opcionales con el premium del simulador como reward (las dos mitades de ese modelo pertenecen al producto anterior) y el planificador como bounded context separado (el BC que separaba se retira).
- **Superados por el sistema de frases** que abrió este viraje (a su vez rehecho el 2026-08-25: [ADR-0082](0082-the-review-captures-the-cursada-in-three-layers.md), [ADR-0083](0083-the-ficha-publishes-counts-not-scores.md), [ADR-0084](0084-free-text-feeds-curation-and-is-never-published.md), [ADR-0085](0085-three-instruments-and-official-data.md)), que hereda su doctrina y retira su superficie: la tasa de aprobación publicada desde el historial privado (publicar un dato con cuántas voces lo sostienen sigue, hoy como conteo y no como proporción ajustada, y su definición de aprobación sigue desde lo que cada reseña declara; el piso de cinco personas no) y los ratings agregados por comisión (se reemplazan por frases con voces; el gate de cobertura para derivar carrera e institución sigue sin umbral: el agregado se condiciona a la cobertura y se publica cuánta hay, pero ningún ADR vigente fija un mínimo). Del piso: acá se dio por retirado, y el modelo vigente volvió a poner uno, de 10 reseñas por cátedra, con la privacidad del que reseña como razón ([ADR-0082](0082-the-review-captures-the-cursada-in-three-layers.md)).
- **Intactos y reforzados por la tesis**: [ADR-0009](0009-review-anonymity-is-a-presentation-rule.md) (anonimato como regla de presentación) y [ADR-0054](0054-a-metric-without-backing-travels-null-never-zero.md) (métrica sin sustento viaja null). El piso de publicación y el condicionamiento por cobertura del modelo vigente son la continuación directa de los dos.
- **Superados por la reseña nueva**: el anclaje de la reseña a un `EnrollmentRecord` (hoy ancla a la cursada, cuenta × materia × período: [ADR-0082](0082-the-review-captures-the-cursada-in-three-layers.md)) y la reseña de texto libre con docente reseñado (ya no existe; la cátedra como sujeto propio sobrevive en el glosario).
- **Superado por el sistema de frases**: el auto-ocultar por umbral de reportes. En el producto nuevo ninguna cantidad de reportes baja nada sola, y sin texto publicado ya no hay reportes de contenido que arbitrar ([ADR-0084](0084-free-text-feeds-curation-and-is-never-published.md)).
- **En retiro junto con la versión anterior**, sin reemplazo directo hasta que el modelo nuevo traiga el suyo: la edición de reseña bloqueada mientras un reporte la modera, la generación de embeddings atada a las transiciones a `Published`, el audit log de reseñas como proyección y la edición destructiva de una cursada que invalida su reseña. Describen código que todavía existe.
- **Diferido a revisión contra la tesis**: [ADR-0039](0039-meilisearch-as-the-global-search-engine.md) (la búsqueda del lector sigue teniendo sentido; el motor dedicado se re-evalúa cuando el producto nuevo la necesite). **Revalidado contra la tesis**: [ADR-0040](0040-notifications-as-a-new-bounded-context.md), porque los avisos sostienen cinco stories del producto nuevo (el que pidió una carrera se entera de que la cargamos; quien aportó se entera antes de que se publique la réplica) y son infraestructura del primer bloque, aunque arranquen solo por mail.
...
- **La sospecha en grupos chicos.** Los datos que la tesis promete (cuánto tarda la gente de verdad, abandono en co-cursada) son más re-identificantes que los anteriores, y el crudo descargable sin registro garantiza que terceros crucen tablas. La posición tomada entonces fue no esconderlo detrás de un piso: no se publica quién, nunca, y ningún umbral elimina la sospecha en un grupo de siete; se le dice al que reseña antes de publicar. El modelo vigente la revisó y sí puso piso, de 10 reseñas por cátedra, con la privacidad del que reseña como razón ([ADR-0082](0082-the-review-captures-the-cursada-in-three-layers.md)). Lo que sí se protege es el texto (moderar lo que expone a una persona) y qué puede citar la réplica.
- **La curaduría hereda el problema del texto libre en otra forma**: texto libre no agrega, frases curadas no cubren. Sin un canal de "ninguna de estas describe lo mío" que alimente la curaduría, el producto responde "¿falló la institución?" solo con las fallas que se nos ocurrieron. ([ADR-0084](0084-free-text-feeds-curation-and-is-never-published.md) lo resuelve: el campo libre existe, no se publica, y de lo que escriben muchos se destilan los ítems que faltaban.)
