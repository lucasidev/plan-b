# Skills nativos de Claude Code

Este es el catálogo que consume Claude Code. Se versiona y mantiene de forma independiente de `.agents/skills/`, que pertenece a Codex. Los propios del proyecto (`slice-*`, `dapper-read`, `integration-event`, `new-adr`, `ship`, `regen-screenshots`, `browser-repro`, `source-research`) se escribieron para planb (`sync-notion` se retiró el 2026-08-18 con Notion). Los de UX/UI son de terceros, copiados como archivos (sin CLI ni ejecución de instaladores) y revisados antes de entrar.

## Investigación acotada

| Skill | Rol en planb |
|---|---|
| `browser-repro` | Reproduce o verifica un síntoma renderizado concreto con una ruta, hasta dos viewports, dos intentos y doce llamadas de browser. |
| `source-research` | Responde una cuestión ordinaria con fuentes públicas, hasta cuatro búsquedas, tres documentos abiertos y seis llamadas web. |
...
| `design-audit` | ídem | MIT | Auditoría visual de lo que ya existe, produce plan por fases. Solo visual, no funcionalidad. | Rutas internas ajustadas al paquete versionado |
| `ui-typography` | ídem (`typography`) | MIT | Tipografía correcta en toda UI generada (comillas, guiones, espaciado, jerarquía). Modo enforcement silencioso. | Directorio renombrado a `ui-typography`; rutas internas ajustadas |
