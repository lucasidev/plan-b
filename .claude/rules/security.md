...
- **Staff/rol**: `.RequireAuthorization(p => p.RequireRole(AdminTeacherPolicy.RoleName))` y sus hermanas por área (`AdminAcademicTermPolicy`, `AdminCareerPlanPolicy`, ...). Nunca hardcodees el string del rol: usá la policy del área que estás tocando.
