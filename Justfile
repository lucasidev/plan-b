...
# Un recorrido de persona (docs/engineering/testing.md) contra el stage real, a mano: sin
# argumento lista las personas disponibles. Secretos en .env.stage.local.
walk persona="":
    bun scripts/run-walk.ts {{persona}}
...
# Lista por default y borra solo con --apply las ramas efímeras de agentes que ya no tienen
# parches únicos respecto de la base elegida. Ejemplo: `just cleanup-agent-branches --base HEAD --apply`.
cleanup-agent-branches *args:
    bun scripts/cleanup-agent-branches.ts {{args}}
