...
/// <summary>
/// Handler unit tests para <see cref="UpholdReportCommandHandler"/>. Cubre la rama
...
/// </summary>
public class UpholdReportCommandHandlerTests
{
    ...
    [Fact]
    public async Task Handle_returns_ReportNotFound_and_does_not_cascade_or_publish_when_report_missing()
    {
        var deps = NewDeps();
        deps.Reports.FindByIdAsync(Arg.Any<ReviewReportId>(), Arg.Any<CancellationToken>())
            .Returns((ReviewReport?)null);

        var result = await UpholdReportCommandHandler.Handle(
            new UpholdReportCommand(Guid.NewGuid(), Guid.NewGuid(), "viola la política"),
            deps.Reports,
            deps.UnitOfWork,
            deps.Bus,
            deps.Clock,
        ...
        // Sin report no hay reseña que cascadear, nada que persistir ni evento que publicar.
        await deps.Reports.DidNotReceive().GetOpenByReviewAsync(
        ...
        await deps.Bus.DidNotReceive().PublishAsync(
            Arg.Any<ReviewRemovalRequestedIntegrationEvent>(), Arg.Any<DeliveryOptions>());
    }
}
