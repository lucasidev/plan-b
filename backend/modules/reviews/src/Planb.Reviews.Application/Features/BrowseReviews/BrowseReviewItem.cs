...
public sealed record BrowseReviewItem(
    ...
    decimal? FinalGrade,
    DateTime CreatedAt,
    // No null marca que el autor editó el texto, y es lo que el feed muestra como "editada". La
    // asimetría importaba: la respuesta del docente exponía sus ediciones y la reseña no, así que
    // el autor podía reescribir el texto después de que le respondieran y dejar la respuesta
    // contestando algo que ya no está escrito, sin que el lector tuviera cómo notarlo.
    //
    // No es UpdatedAt: esa marca la mueven también la respuesta del docente y las transiciones de
    // moderación, o sea acciones de otra gente sobre la reseña.
    DateTime? EditedAt,
