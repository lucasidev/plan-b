...
///   <item><see cref="CourseReview.Create"/> valida el resto y arma el aggregate. El SaveChanges lo
...
/// </para>
/// </summary>
public static class PublishCourseReviewCommandHandler
{
    public static async Task<Result<PublishCourseReviewResponse>> Handle(
        PublishCourseReviewCommand command,
        ...
        CancellationToken ct)
    {
        ...
        if (subject is null)
        {
            return CourseReviewErrors.SubjectNotFound;
        }

        // 2) El período existe.
        if (!await academic.AcademicTermExistsAsync(command.TermId, ct))
        {
            return CourseReviewErrors.TermNotFound;
        }
        ...
        if (command.ChairId is { } chairId)
        {
            ...
            {
                return CourseReviewErrors.ChairNotInSubject;
            }
        }
        ...
        {
            return CourseReviewErrors.NoCurrentInstrument;
        }
        ...
        foreach (var answer in command.Answers)
        {
            ...
            {
                return CourseReviewErrors.ItemNotInInstrument;
            }
        ...
        {
            return CourseReviewErrors.AlreadyReviewed;
        }

        // 6) El aggregate valida el resto y se arma.
        var created = CourseReview.Create(
            command.UserId,
            command.SubjectId,
            command.TermId,
            command.ChairId,
        ...
        return new PublishCourseReviewResponse(review.Id.Value, review.Answers.Count);
    }
}
