...
/// <summary>
/// POST /api/reviews/courses (US-146): el acto de reseñar una cursada.
///
/// <para>
...
public sealed class PublishReviewEndpoint : ICarterModule
{
    ...
    {
        app.MapPost("/api/reviews/courses", async (
            ...
            CancellationToken ct) =>
        {
            ...
            var command = new PublishReviewCommand(
                ...
                body.ChairId,
                (body.Answers ?? [])
                    .Select(a => new ReviewAnswerInput(a.ItemCode, a.OptionValue))
                    .ToList(),
                body.FreeText);
            ...
            try
            {
                ...
                if (result.IsSuccess)
                {
                    return Results.Created(
                        $"/api/reviews/courses/{result.Value.Id}", result.Value);
                }
