...
/// </para>
/// </summary>
public sealed class ReviseReviewEndpoint : ICarterModule
{
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        app.MapPut("/api/reviews/courses/{id:guid}", async (
            Guid id,
            ReviseReviewRequest body,
            HttpContext http,
            IMessageBus bus,
            CancellationToken ct) =>
        {
            ...
            var command = new ReviseReviewCommand(
                userId.Value,
                id,
                (body.Answers ?? [])
            ...
            try
            {
                var result = await bus.InvokeAsync<Result<ReviseReviewResponse>>(command, ct);
                if (result.IsSuccess)
                {
        ...
        .WithName("Reviews_ReviseReview")
        .WithTags("Reviews")
        .RequireAuthorization()
        .Produces<ReviseReviewResponse>(StatusCodes.Status200OK)
