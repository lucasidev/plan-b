...
/// <summary>
/// Write-side del aggregate <see cref="CourseReview"/> (US-146, ADR-0082). Los conteos que la ficha
...
{
    Task AddAsync(CourseReview review, CancellationToken ct = default);

    /// <summary>Carga el aggregate con sus respuestas para editarlo. Null si no existe.</summary>
    Task<CourseReview?> GetByIdAsync(CourseReviewId id, CancellationToken ct = default);
    ...
    /// </summary>
    Task<CourseReview?> GetByCursadaAsync(
        Guid accountId,
        Guid subjectId,
        Guid termId,
