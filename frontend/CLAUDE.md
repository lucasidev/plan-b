...
- **No** construir contra nada que no sea la story. La UI de una US sale de su "listo cuando" y de la persona a la que sirve; los tokens, de `design-system.md`. El mapa mid-fi (`docs/design/map/`) es orientativo sobre qué vistas y flujos podría haber, no un contrato de UX/UI: se puede apartar de él sin pedir permiso. Las capturas del canvas de la versión anterior (`docs/design/reference/screenshots/`) están congeladas: no son referencia para trabajo nuevo.
