 ...
 * la cascada de universidad y período: la cátedra es de una materia y persiste entre períodos,
 * así que colgarla de un período modelaría mal lo que se está cargando.
 *
...
export default async function AdminChairsPage({ searchParams }: Props) {
  ...
  if (subjectId && !z.string().uuid().safeParse(subjectId).success) notFound();
  const context = subjectId ? await fetchChairSubjectContextServer(subjectId) : null;
  if (subjectId && !context) notFound();
    ...
    <div className="mx-auto w-full max-w-[720px] px-4 py-8">
      ...
      <div className="mb-5">
        ...
        <p className="text-[13px] leading-relaxed text-ink-2">
          {context
            ? context.subjectName
            : 'Elegí una materia para consultar sus cátedras y gestionar el equipo docente.'}
        </p>
      </div>
      ...
      {!subjectId ? (
        ...
        <div className="flex flex-col gap-5">
          {context?.subjectIsActive ? (
            <CreateChairForm subjectId={subjectId} />
          ) : (
            <p className="text-[13px] text-ink-2">
              Materia archivada. Podés consultar las cátedras existentes.
            </p>
          )}
