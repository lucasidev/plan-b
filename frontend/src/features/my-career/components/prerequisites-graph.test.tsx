...
import type { GraphEdge, GraphNode } from '@/features/my-career/data/prerequisites-mock';
import { PrerequisitesGraph } from './prerequisites-graph';
...
describe('PrerequisitesGraph', () => {
  it('renderea un grupo SVG por cada nodo del mock', () => {
    const { container } = render(
      <PrerequisitesGraph nodes={fixtureNodes} edges={fixtureEdges} focusId={null} />,
    );
  ...
  it('muestra leyenda con los 5 estados visuales', () => {
    render(<PrerequisitesGraph nodes={fixtureNodes} edges={fixtureEdges} focusId={null} />);
  ...
  it('cada nodo es un link al drawer /my-career/subject/[id]', () => {
    render(<PrerequisitesGraph nodes={fixtureNodes} edges={fixtureEdges} focusId={null} />);
  ...
  it('ignora aristas a nodos inexistentes', () => {
    ...
    const { container } = render(
      <PrerequisitesGraph nodes={fixtureNodes} edges={edgesConGhost} focusId={null} />,
    );
  ...
  it('cuando hay focus, el banner menciona la materia y las que habilita', () => {
    render(<PrerequisitesGraph nodes={fixtureNodes} edges={fixtureEdges} focusId="B" />);
