...
// Carousel deslizante con UNA sola animación sobre el track: las caras son secciones
// físicas de una tira horizontal, así dos caras no pueden superponerse ni puede quedar
// una ventana vacía (los dos modos de falla del crossfade anterior, que dependía de
// cuatro animaciones perfectamente sincronizadas por delay). Ciclo de 20s, ~4.2s por
// cara + 0.8s de deslizamiento. La primera cara se repite al final del track para que
// el salto del loop (100% -> 0%) caiga sobre contenido idéntico y sea invisible.
const CAROUSEL_CSS = `
.demo-planner-track {
  display: flex;
  height: 100%;
  animation: demo-planner-slide 20s ease-in-out infinite;
}
.demo-planner-slide {
  flex: 0 0 100%;
  min-width: 0;
  /* Aire entre caras adyacentes durante el deslizamiento; va dentro del 100% para no
     romper la aritmética del translateX. */
  padding: 0 8px;
}
@keyframes demo-planner-slide {
...
@media (prefers-reduced-motion: reduce) {
  .demo-planner-track { animation: none; }
}
`;
 ...
 * fetch ni JS de cliente; con `prefers-reduced-motion` queda fija la semana.
 */
export function DemoPlanner() {
  return (
    <div className="bg-bg" style={{ borderRadius: 10, padding: '12px 14px' }}>
      ...
      <div className="overflow-hidden" style={{ height: 132 }}>
        <div className="demo-planner-track">
          <WeekSlide />
          ...
          {/* Copia de la primera cara: el reinicio del loop cae sobre contenido idéntico. */}
          <WeekSlide />
        </div>
      </div>
    </div>
