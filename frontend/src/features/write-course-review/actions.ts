...
import type { PublishCourseReviewResult } from './types';
 ...
 */
export async function publishCourseReviewAction(
  _prev: PublishCourseReviewResult,
  formData: FormData,
): Promise<PublishCourseReviewResult> {
  ...
  const response = await apiFetchAuthenticated('/api/reviews/cursadas', {
    method: 'POST',
    body: JSON.stringify({
      subjectId,
      termId,
