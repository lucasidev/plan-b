...
import type { TeacherReviewsPage } from '../types';
import { TeacherReviewCard } from './teacher-review-card';

/**
 * Reviews list for the teacher page (US-003): section heading + the page of cards + a link-based
 * pager (server-driven pagination via `?page=N`, SEO-friendly for a public page). These are the
 * reviews where the teacher was the `docente_reseñado`, across all subjects.
 */
    ...
    return (
      <section className="rounded-lg border border-line bg-bg-card p-10 text-center">
        <p className="font-display text-lg font-semibold text-ink m-0">
          Este docente todavía no tiene reseñas.
        </p>
        <p className="mt-2 text-sm text-ink-3">
          Cuando alguien reseñe una cursada con este docente, va a aparecer acá.
        </p>
      </section>
    );
  }
