 ...
 */
export async function reviseCourseReviewAction(
  reviewId: string,
  answers: Record<string, number>,
  freeText: string | null,
): Promise<ActionState> {
  const response = await apiFetchAuthenticated(`/api/reviews/cursadas/${reviewId}`, {
    method: 'PUT',
    body: JSON.stringify({
 ...
 */
export async function deleteCourseReviewAction(reviewId: string): Promise<ActionState> {
  const response = await apiFetchAuthenticated(`/api/reviews/cursadas/${reviewId}`, {
    method: 'DELETE',
  });
