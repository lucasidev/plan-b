...
import type { CommissionSelection, SimulationEvaluation } from '../types';
...
function selections(
  subjectIds: string[],
  commissionId: string | null = null,
): CommissionSelection[] {
  return subjectIds.map((subjectId) => ({ subjectId, commissionId }));
}

/**
 * Renderea el panel con la respuesta ya sentada en el cache (mismo patrón que
 * subject-picker-drawer.test.tsx): `simulationEvaluationQueries.forSelections` ordena y arma la
 * key adentro, así que sembrar con la misma función garantiza la key exacta que el componente pide.
 * `response` en `undefined` deja el cache vacío (para el caso sin materias elegidas, donde el
 * query ni se dispara).
 */
function renderPanel(sel: CommissionSelection[], response: SimulationEvaluation | undefined) {
  const queryClient = new QueryClient({
    defaultOptions: { queries: { retry: false, staleTime: Number.POSITIVE_INFINITY } },
  });
  if (response) {
    queryClient.setQueryData(simulationEvaluationQueries.forSelections(sel).queryKey, response);
  }
  ...
  return render(<SimulatorEvaluationPanel selections={sel} />, { wrapper });
}
...
describe('SimulatorEvaluationPanel (US-016)', () => {
  ...
  it('muestra las métricas cuando la combinación es válida y la muestra alcanza', () => {
    renderPanel(selections(['sub-1', 'sub-2']), evaluation());
  ...
  it('dificultad null muestra "sin reseñas todavía", nunca un 0', () => {
    // clashes: 0 (no null) aísla el caso: sin esto, el tile de choques también cae en "sin datos"
    // (sin comisión elegida) y el getByText sería ambiguo entre los dos tiles.
    renderPanel(
      selections(['sub-1'], 'com-1'),
      evaluation({ weightedDifficulty: null, clashes: 0 }),
    );
  ...
  describe('métrica de choques (US-096)', () => {
    it('clashes null: no hay comisión elegida, el dato honesto es "no sabemos"', () => {
      renderPanel(selections(['sub-1']), evaluation({ clashes: null }));

      expect(screen.getByText('sin comisión elegida')).toBeInTheDocument();
      expect(screen.queryByText(/^\d+$/)).not.toBeInTheDocument(); // ningún número inventado de choques
    });

    it('clashes 0: se muestra el número real, no se esconde', () => {
      renderPanel(selections(['sub-1'], 'com-1'), evaluation({ clashes: 0 }));

      expect(screen.getByText('0')).toBeInTheDocument();
      expect(screen.getByText('choques')).toBeInTheDocument();
    });

    it('clashes > 0: singular "choque" cuando es exactamente 1', () => {
      renderPanel(selections(['sub-1'], 'com-1'), evaluation({ clashes: 1 }));

      expect(screen.getByText('1')).toBeInTheDocument();
      expect(screen.getByText('choque')).toBeInTheDocument();
    });

    it('clashes > 1: plural "choques"', () => {
      renderPanel(selections(['sub-1', 'sub-2'], 'com-1'), evaluation({ clashes: 2 }));
